#
# Revit Batch Processor — Revit script host (IronPython 3.4 port)
#
# Phase 2b: full port of Scripts/revit_script_host.py. Same structure, same
# call sequence, same orchestrator protocol. Py2→3 fixes:
#   - `except X, e:` → `as e:`
#   - `xrange` → `range`
#
...
def RunSingleTaskScript(scriptFilePath):
    ...
    output()
    output("Operation aborted." if aborted else "Operation completed.")
    return aborted
...
def RunBatchTaskScript(scriptFilePath):
    ...
    if enableDataExport and not path_util.DirectoryExists(dataExportFolderPath):
        ...
        if enableDataExport:
            ...
            snapshot_data_exporter.ExportTemporarySnapshotData(
                sessionId, centralFilePath, isCloudModel,
        ...
        localFilePath = None
        openCreateNewLocal = False  # Default False because the file may not be a workshared Central file.
        ...
        try:
            ...
            def processDocument(doc):
                ...
                return script_host_error.WithErrorHandling(
                    executeTaskScript,
                    "ERROR: An error occurred while executing the task script! Operation aborted.",
                    output,
                    showMessageBoxOnTaskError,
            ...
            if activeDoc is not None:
                ...
                revit_script_util.RunCloudDocumentAction(
                    uiapp, openInUI, cloudProjectId, cloudModelId,
                    worksetConfigurationOption, auditOnOpening, processDocument, output,
                )
                ...
                revit_script_util.RunNewLocalDocumentAction(
                    uiapp, openInUI, centralFilePath, localFilePath,
                    worksetConfigurationOption, auditOnOpening, processDocument, output,
                )
            elif isCentralModel or isLocalModel:
                revit_script_util.RunDetachedDocumentAction(
                    uiapp, openInUI, centralFilePath, discardWorksetsOnDetach,
                    worksetConfigurationOption, auditOnOpening, processDocument, output,
                )
            else:
                revit_script_util.RunDocumentAction(uiapp, openInUI, centralFilePath, auditOnOpening, processDocument, output)
            ...
            if enableDataExport:
                ...
                snapshot_data_exporter.ExportSnapshotData(
                    sessionId, centralFilePath, isCloudModel,
                    cloudProjectId, cloudModelId,
                    snapshotStartTime, snapshotEndTime,
                    dataExportFolderPath, revitJournalFilePath, snapshotError,
...
def ShutdownSession(process):
    for _ in range(CLOSE_MAIN_WINDOW_ATTEMPTS):
        process.Refresh()
...
def DoRevitSessionProcessing(scriptFilePath, scriptDataFilePath, progressNumber, batchRvtProcessUniqueId, output):
    results = []
    ...
    if len(scriptDatas) > 0:
        ...
        if revitProcessingOption == BatchRvt.RevitProcessingOption.BatchRevitFileProcessing:
            ...
            revit_script_util.SetCurrentScriptData(scriptData)
            results.append(RunSingleTaskScript(scriptFilePath))
    else:
...
def Main():
    ...
    if outputPipeHandleString is None or scriptFilePath is None:
        return
